...
clear all;
close all;
%% import data, keep only Hs, Tp and wind speed U （原来的代码，此段不运行）
...
%data = [U Hs Tp]; % data: U, Hs, Tp %原代码中的data数据
%}
%% import china xi sha qun dao data (该段为新的数据加载）

data1 = load('E:\Simulation_File\windWaveAnalysis\analysisFile\data.mat');
data = data1.data;
U = data(:,1);
Hs = data(:,2);
Tp = data(:,3);
